...
public class LeaseAgreementController {
    ...
    public Result<List<AgreementItemVo>> listItem() {
        ...
        List<AgreementItemVo> result=service.listByPhone(phone);
    ...
    public Result<AgreementDetailVo> getDetailById(@RequestParam Long id) {
        AgreementDetailVo result=service.getgetDetailById(id);
        return Result.ok(result);
    }

    @Operation(summary = "根据id更新租约状态", description = "用于确认租约和提前退租")
    @PostMapping("updateStatusById")
    public Result updateStatusById(@RequestParam Long id, @RequestParam int leaseStatus) {
        LambdaUpdateWrapper<LeaseAgreement> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(LeaseAgreement::getId, id);
        updateWrapper.set(LeaseAgreement::getStatus, LeaseStatus.valueOfCode(leaseStatus));
